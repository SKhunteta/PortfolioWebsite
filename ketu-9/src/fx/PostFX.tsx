...
// ocean glint lane, the leviathan photophores, additive spray/waterfalls —
// and the base grade (tuned long before this pass existed) stays untouched.
// DoF is driven by the Observer director through `dofChannel`: the effect
// never unmounts (rebuilding the composer hitches); instead bokehScale damps
// to zero whenever no shot wants focus.
//
...
function Effects() {
  ...
  return (
    <EffectComposer multisampling={0} frameBufferType={HalfFloatType}>
      <SMAA />
      <Bloom mipmapBlur intensity={0.55} luminanceThreshold={1.05} luminanceSmoothing={0.15} />
      <DepthOfField ref={dofRef} bokehScale={0} worldFocusDistance={100} worldFocusRange={18} />
